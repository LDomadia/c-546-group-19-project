...
//gets all occurrences of specific cloth type in clothes db
...
//if similarities between each clothing piece match the preferences to a certain threshold, recommend the highest similarity
//returns an object with the ID of each best matching cloth, can return nothing for a certain piece/s
//assumption is parameters are all given (except optional threshold), and are valid based on the clothes db
async function generateOutfit(
  ...
  for (let i = 0; i < cloth_names.length; i++) {
    existingClothes[cloth_names[i]] = await getClothType(
      clothesCollection,
      cloth_names[i],
      user
    );
  ...
  for (let i = 0; i < cloth_names.length; i++) {
    scoreClothes[cloth_names[i]] = existingClothes[cloth_names[i]].map(
      (cloth) => matchClothPreferences(prefs, cloth)
    );
    bestClothes[cloth_names[i]] = scoreClothes[cloth_names[i]].reduce(
      (scoreA, scoreB) => (scoreA.score > scoreB.score ? scoreA : scoreB),
      -1
    );
