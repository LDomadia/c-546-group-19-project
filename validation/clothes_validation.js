...
function checkSelectInput(input, inputName, acceptableValues) {
  ...
  if (!acceptableValues.includes(input))
    throw `Error: ${inputName} contains invalid inputs.`;
  return input;
}
...
function checkListInput(input, inputName) {
  ...
  if (setInput.length != input.length)
    throw `Error: ${inputName} contains duplicates`;
...
function checkCheckboxInput(input, inputName, acceptableValues) {
  ...
  input = input.map((element) => {
    ...
    if (!acceptableValues.includes(element))
      throw `Error: ${inputName} contains invalid inputs.`;
    return element;
  });
  return input;
